...
struct Post: Identifiable, Codable {
    ...
    let linkedMarketUpdate: MarketUpdate?
    let images: [String]  // URLs or image data
    let isAutoGenerated: Bool
    
    var userHandle: String {
        // If author is already in handle form (starts with @), preserve as-is
        if author.hasPrefix("@") { return author }
        // If author looks like a handle (no spaces), preserve casing and just prefix '@'
        if !author.contains(" ") { return "@\(author)" }
        // Otherwise, derive a handle-like string (lowercased, no spaces)
        return "@\(author.replacingOccurrences(of: " ", with: "").lowercased())"
    }
    ...
    enum CodingKeys: String, CodingKey {
        case id, author, role, content, timestamp, isSponsored, linkedOpportunity, linkedInvestment, linkedMarketUpdate, images, isAutoGenerated
    }
    ...
    init(id: UUID = UUID(), 
         ...
         linkedMarketUpdate: MarketUpdate? = nil,
         images: [String] = [],
        ...
        self.linkedMarketUpdate = linkedMarketUpdate
        self.images = images
